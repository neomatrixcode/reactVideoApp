...
class FFmpegWrapper {
  static convertVideo(
    ...
    const ffmpegCommand1 = `-hide_banner -i ${videoURI} -vf "fps=30/1:round=up,scale=720:-2" -y -c:v libx264 -r 30 -g 240 -b:a 48k ${outputVideoPath}`;

    const ffmpegCommand = `-hide_banner -i ${videoURI} -vf "fps=30/1:round=up,scale=720:-2" -y -c:v mpeg4 -r 30 -g 240 -b:a 48k ${outputVideoPath}`;
